...
public class AIAdvance extends AI {
	...
	public ArrayList<Integer> path = new ArrayList();// đường đi ngắn nhất
        public int col = Game.HEIGHTTile; // cột dọc
        public int row = Game.WIDTHTile;	// cột ngang

        public int[][] node = new int[col * row][4];  //ma trận đỉnh kề
        public int numOfNode = col * row;	 // số lượng đỉnh tối đa
        public int[][] matrix = new int[col][row];  // ma trận đỉnh gồm cái số nguyên âm 0 và nguyên dương. ma trận 2 chiều
    ...
    public  void getMatrix(){
	    int nameOfVertext = 1;
            for (int i = 0; i < col; i++){
                   for (int j = 0; j < row; j++){
                       ...
                       else if (Map.map[i][j] =='*' || Map.map[i][j] == 'x'
                               || Map.map[i][j] == 'b' || Map.map[i][j] == 'f'
                               || Map.map[i][j] == 's'|| Map.map[i][j] == 'r'){
        ...
        */
        ...
        public void updateDestroy_Brick(){
            if ( Brick.xOfAllBrick.isEmpty() ) return;
            
            for (int i = 0; i < Brick.xOfAllBrick.size(); i++ ){
                int Xgach = Brick.xOfAllBrick.get(i);
                int Ygach = Brick.yOfAllBrick.get(i);
    ...
    */
        ...
        //update vị chí bom 
        int r =Game.getPresentPower (); // bán kính vụ nổi
        // toa độ enemy
